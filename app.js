...
function prepareIntakeControls() {
  ...
  intakeControls().forEach(control => {
    ...
    const saved = intakeData[key] ?? (control.name === "current_health_concern" ? intakeData.current_priority_symptom : undefined);
    if (saved !== undefined) {
  ...
  updateYogaTeacherRequirement();
}
function updateYogaTeacherRequirement() {
  const teacher = document.getElementById("yoga-location-teacher");
  const practicesYoga = document.querySelector('input[name="yoga_practice"][value="yes"]')?.checked;
  if (!teacher) return;
  teacher.required = Boolean(practicesYoga);
  teacher.setAttribute("aria-required", String(Boolean(practicesYoga)));
  teacher.closest(".form-group").querySelector(".conditional-required").hidden = !practicesYoga;
}
...
async function init() {
  ...
  prepareIntakeControls();
  document.querySelectorAll('input[name="yoga_practice"]').forEach(control => control.addEventListener("change", updateYogaTeacherRequirement));
  syncDobDisplay();
