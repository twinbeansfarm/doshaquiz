...
const t = {
  en: {
    ...
    copy: "Copy link", copied: "Profile link copied", share: "Share", shareProfile: "Share profile", sharedCopy: "Profile link copied", print: "Print profile", reset: "Retake quiz",
  ...
  vi: {
    ...
    copy: "Sao chép liên kết", copied: "Đã sao chép liên kết hồ sơ", share: "Chia sẻ", shareProfile: "Chia sẻ hồ sơ", sharedCopy: "Đã sao chép liên kết hồ sơ", print: "In hồ sơ", reset: "Làm lại",
    ...
    shared: "Hồ sơ Ayurveda được chia sẻ", takeOwn: "Làm hồ sơ của tôi", printIntake: "In Hồ sơ y tế", printDate: "Ngày in", language: "Ngôn ngữ",
...
function updateIntakeLanguage() {
  ...
  document.getElementById("copy-intake-btn").textContent = t[currentLang].copy;
...
function prepareIntakeControls() {
  ...
  document.getElementById("copy-intake-btn").hidden = isSharedView;
...
async function sharePdf(kind) {
  ...
  const filename = `TwinBeansFarm_${kind === "intake" ? "Health" : "Ayurveda"}_Profile_${safeFilename(selections.name)}_${new Date().toISOString().slice(0, 10)}.pdf`;
  try {
...
async function saveShared() {
  const response = await fetch("/.netlify/functions/saveQuiz", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(combinedProfile()) });
...
async function exportToText(source = "quiz") {
  try {
    await copyText(await saveShared());
    const button = document.getElementById(source === "intake" ? "copy-intake-btn" : "copy-btn");
    if (button) button.textContent = t[currentLang].copied;
    showActionFeedback(t[currentLang].copied, false, source);
  } catch (error) {
    console.error("Unable to copy shared profile", error);
    showActionFeedback(t[currentLang].shareError, true, source);
  }
}
